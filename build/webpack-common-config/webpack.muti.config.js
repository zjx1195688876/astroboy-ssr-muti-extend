...
for (let pageName in PAGE_ROUTER) {
  ...
  let cConfig = merge({}, clientConfig, {
    entry: {
      [pageName]: `${webpackConfig.ssr}/${pageName}/entry-client.js`        //buildEntryFiles生成的配置文件
    },
    output: {
    ...
    plugins: [
      new VueSSRClientPlugin({
        filename: `server/${pageName}/vue-ssr-client-manifest.json`//static的build目录
      })
    ]
  });
  let sConfig = merge({}, serverConfig, {
    entry: {
      [pageName]: `${webpackConfig.ssr}/${pageName}/entry-server.js`        //buildEntryFiles生成的配置文件
    },
    output: {
      path: webpackConfig.static ? webpackConfig.static : path.join(ROOT_PATH, '/static/build/'),
    },
    plugins: [
      new VueSSRServerPlugin({
        filename: `server/${pageName}/vue-ssr-server-bundle.json`       //static的build目录
      })
    ]
  });
