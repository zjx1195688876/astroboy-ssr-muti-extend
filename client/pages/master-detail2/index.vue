...
export default {
  ...
  // 避免在客户端重复注册模块。
  destroyed () {
    this.$store.unregisterModule('masterDetail2')
  },
  data() {
    return {
      avatar: null,
  ...
  methods: {
    dispatch(store, url) {
      ...
      store.registerModule('masterDetail2', masterDetailStoreModule);
      return Promise.all([
        store.dispatch('masterDetail2/getTeacherInfo', { teacherId }),
        store.dispatch('masterDetail2/getCourseList', { teacherId, pageNumber: 1 })
      ]);
    },
    getState() {
      if (this.$store.state.masterDetail2) {
        this.teacherInfo = this.$store.state.masterDetail2.teacherInfo;
        this.courseInfo = this.$store.state.masterDetail2.courseInfo;
    ...
    parseTeacherInfo() {
      ...
      /**
       * 在拿到数据之后进行判断描述信息是否需要隐藏部分
       * 需要匹配字符串，然后让文字超过三行能够隐藏后续内容
       */
      // if (description) {
      //   const eleDesWidth = document.querySelector('.master-info-des').offsetWidth;
      //   const isOverWrite = judgeOverWrite(description, eleDesWidth);
      //   this.isOverWrite = isOverWrite;
      //   this.clipDes = isOverWrite;
      // }
