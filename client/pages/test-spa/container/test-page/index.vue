...
export default {
  ...
  // 避免在客户端重复注册模块。
  destroyed () {
    this.$store.unregisterModule('testPage')
  },
  data() {
    return {
      avatar: null,
  ...
  methods: {
    dispatch(store, url) {
      ...
      store.registerModule('testPage', masterDetailStoreModule);
      return Promise.all([
        store.dispatch('testPage/getTeacherInfo', { teacherId }),
        store.dispatch('testPage/getCourseList', { teacherId, pageNumber: 1 })
      ]);
    },
    getState() {
      if (this.$store.state.testPage) {
        this.teacherInfo = this.$store.state.testPage.teacherInfo;
        this.courseInfo = this.$store.state.testPage.courseInfo;
